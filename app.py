...
# --- THE SURGICAL PATCH (Universal Compatibility) ---
...
# 1. PAGE CONFIG & BRANDING (Surgically Fixed)
...
if st.button("🚀 PHASE 1 & 2: GENERATE SCRIPT"):
    with st.status("🔬 Analyzing medical data...", expanded=True) as status:
        prompt = (f"Act as a Cardiac Physician. Research {topic}. "
                  "Write a 35s viral Hinglish script for Reels. "
...
if st.session_state.script:
    ...
    if st.button("🎬 PHASE 3: RENDER VIDEO"):
        with st.spinner("⚡️ Creating HD Video... (60-90 seconds)"):
            try:
                # A. VOICE OVER
                v_file = "v.mp3"
                ...
                with open("f.mp4", "rb") as file:
                    st.download_button("📥 DOWNLOAD TO PHONE", file, file_name="Gurjas_Medical_Short.mp4")
