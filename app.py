"""
GURJAS V12 — DR. VASU SUSPENSE STORY ENGINE
Fix: Bulletproof scene parser handles ALL LLM output formats
"""
...
# ─────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="GURJAS V12 | Suspense Studio",
    page_icon="🏥",
    layout="wide"
)
...
.stApp {
    background: radial-gradient(ellipse at top, #080f1f 0%, #000308 70%);
    color: #e8e8e8;
    font-family: 'Rajdhani', sans-serif;
}
.hdr { text-align:center; padding:2.4rem 1rem 1.4rem; border-bottom:1px solid rgba(255,215,0,.18); margin-bottom:2rem; }
.hdr h1 {
    font-family:'Cinzel',serif; font-size:2.7rem; font-weight:900; letter-spacing:.13em;
    background:linear-gradient(135deg,#FFD700 0%,#FFA500 50%,#FFD700 100%);
    -webkit-background-clip:text; -webkit-text-fill-color:transparent;
}
.hdr p { color:#778899; font-size:.93rem; letter-spacing:.28em; text-transform:uppercase; margin-top:.4rem; }
.card {
    background:linear-gradient(135deg,rgba(255,215,0,.04),rgba(255,140,0,.02));
    border:1px solid rgba(255,215,0,.14); border-radius:14px;
    padding:1.8rem 2rem; margin:1.5rem 0; position:relative;
}
.badge {
    position:absolute; top:-13px; left:20px; background:#FFD700; color:#000;
    font-family:'Cinzel',serif; font-weight:700; font-size:.69rem;
    letter-spacing:.2em; padding:3px 16px; border-radius:20px;
}
.stButton>button {
    width:100%; background:linear-gradient(135deg,#FFD700,#FFA500); color:#000;
...
.mbox {
    flex:1; min-width:90px; background:rgba(255,215,0,.06);
    border:1px solid rgba(255,215,0,.18); border-radius:9px; padding:1rem; text-align:center;
}
...
.arc-step { padding:5px 14px; border-radius:20px; font-size:.78rem; font-weight:600; letter-spacing:.08em; }
[data-testid="stSidebar"] { background:rgba(3,6,14,.96); border-right:1px solid rgba(255,215,0,.1); }
.debug-box { background:rgba(0,100,0,.15); border:1px solid #0a0; border-radius:8px; padding:1rem; margin:.5rem 0; font-family:monospace; font-size:.85rem; }
</style>
...
PROJECT_DIR = "gurjas_v12"

for k, v in [("package", ""), ("scenes", []), ("angles", ""), ("raw_llm", "")]:
...
def parse_scenes(raw_text, fallback_phases):
    """
    Handles every possible LLM output format:
      SCENE 1 | HOOK | keyword | hindi text
      SCENE | hook | keyword | hindi text
      SCENE | keyword | hindi text
      1. HOOK: keyword — hindi text
      Scene 1: hook | keyword | hindi
    Returns list of dicts: {phase, keyword, hindi}
    """
    scenes = []

    # ── FORMAT A: SCENE {number} | {phase} | {keyword} | {hindi}  (your exact error case)
    matches = re.findall(
    ...
    # ── FORMAT B: SCENE | {keyword} | {hindi}  (2-column, no phase)
    matches = re.findall(
        r"SCENE\s*\d*\s*\|\s*([^|\n]+?)\s*\|\s*([^\n]+)",
        raw_text, re.IGNORECASE
    )
    ...
    # ── FORMAT C: numbered lines  "1. keyword — hindi"
    matches = re.findall(
        r"^\s*\d+[\.\)]\s*([^\-—\n]{3,40})\s*[\-—]\s*([^\n]{10,})",
    ...
    # ── FORMAT D: any line with a pipe — desperate fallback
    lines = [l.strip() for l in raw_text.split("\n") if "|" in l and len(l) > 20]
    for i, line in enumerate(lines[:12]):
        parts = [p.strip() for p in line.split("|") if p.strip()]
        if len(parts) >= 3:
            # Try to find phase word
            phase_word = parts[0].lower() if parts[0].lower() in VALID_PHASES else (
                         parts[1].lower() if parts[1].lower() in VALID_PHASES else "clue")
            # keyword = first non-phase part
            kw_idx = next((j for j, p in enumerate(parts) if p.lower() not in VALID_PHASES
                           and not p.lower().startswith("scene")), 1)
            keyword = parts[kw_idx] if kw_idx < len(parts) else parts[-2]
            hindi   = parts[-1]
            phase   = fallback_phases[i] if i < len(fallback_phases) else "clue"
...
def build_phase_list(n):
    """Distribute story phases across n scenes."""
    if n <= 4:
        return ["hook", "clue", "reveal", "resolution"][:n]
    elif n == 5:
        return ["hook", "mystery", "clue", "reveal", "resolution"]
    elif n == 6:
        return ["hook", "mystery", "clue", "clue", "reveal", "resolution"]
    elif n == 7:
        return ["hook", "mystery", "clue", "rising", "clue", "reveal", "resolution"]
    elif n == 8:
        return ["hook", "mystery", "clue", "rising", "clue", "rising", "reveal", "resolution"]
    elif n == 9:
        return ["hook", "mystery", "clue", "rising", "clue", "rising", "clue", "reveal", "resolution"]
    else:
        base = ["hook", "mystery", "clue", "rising", "clue", "rising", "clue", "rising", "reveal", "reveal", "resolution", "resolution"]
        extras = ["clue", "rising"] * 6
        return (base + extras)[:n]


# ─────────────────────────────────────────────────────────────────
# CARTOON ENGINE CONSTANTS
# ─────────────────────────────────────────────────────────────────
W, H = 1080, 1920
...
CYCLE_ORDER = [
    "heart", "brain", "lungs", "dna", "blood_cells", "pill",
    "surgery", "skeleton", "virus", "hospital", "warning",
    "microscope", "xray", "injection", "detective", "reveal_burst"
]
...
def draw_heart(draw, ac, tx):
    ...
    for g in range(8, 0, -1):
        r = 260 + g * 24
        draw.ellipse([cx-r, cy-r, cx+r, cy+r], fill=tuple(max(0, c//6) for c in ac))
...
def draw_brain(draw, ac, tx):
    cx, cy = W // 2, H // 2 - 80
    ac2 = tuple(max(0, c-40) for c in ac)
    draw.ellipse([cx-260, cy-220, cx+20,  cy+210], fill=ac)
    draw.ellipse([cx-20,  cy-220, cx+260, cy+210], fill=ac2)
    for i in range(7):
        ang = math.pi * i / 7
...
def draw_pill(draw, ac, tx):
    ...
    for i in range(6):
        ang = 2*math.pi*i/6
        mx = cx + int(310*math.cos(ang)); my = cy + int(310*math.sin(ang))
...
def draw_injection(draw, ac, tx):
    ...
    for i in range(5):
        ang = 2*math.pi*i/5
        mx = cx + int(260*math.cos(ang)); my = cy + int(260*math.sin(ang))
...
SCENE_DRAWERS = {
    ...
    "reveal_burst": draw_reveal_burst,
}


def pick_scene(keyword, phase, idx):
    kw = keyword.lower()
    if any(w in kw for w in ["heart","cardiac","attack","chest","coronary","artery","myocard","infarct"]): return "heart"
    if any(w in kw for w in ["brain","stroke","neuro","memory","headache","migraine","alzheimer"]):         return "brain"
    if any(w in kw for w in ["lung","breath","asthma","oxygen","respiratory","pulmon"]):                    return "lungs"
    if any(w in kw for w in ["dna","gene","cancer","tumor","chromosome","heredit"]):                       return "dna"
    if any(w in kw for w in ["blood","cell","hemoglobin","anemia","clot","platelet","pressure","bp"]):     return "blood_cells"
    if any(w in kw for w in ["medicine","pill","drug","tablet","treatment","dose","pharmac"]):              return "pill"
    if any(w in kw for w in ["surgery","operation","bypass","transplant","surgical"]):                      return "surgery"
    if any(w in kw for w in ["bone","joint","fracture","calcium","osteo","spine","disc"]):                  return "skeleton"
    if any(w in kw for w in ["virus","bacteria","infection","fever","immune","covid","flu"]):               return "virus"
    if any(w in kw for w in ["hospital","ward","icu","patient","bed","admit","cardio","echo","echocard"]): return "hospital"
    if any(w in kw for w in ["warning","sign","symptom","danger","risk","alert","silent","fail"]):          return "warning"
    if any(w in kw for w in ["test","lab","biopsy","pathology","diagnosis","examine"]):                     return "microscope"
    if any(w in kw for w in ["xray","scan","mri","ct","imaging","x-ray","radiolog"]):                      return "xray"
    if any(w in kw for w in ["inject","vaccine","insulin","iv","syringe","drip"]):                         return "injection"
    # Phase fallback
    phase_defaults = {
        "hook": "warning", "mystery": "detective", "clue": "microscope",
        "rising": "xray", "reveal": "reveal_burst", "resolution": "hospital"
    }
...
def build_frame(keyword, phase, idx, total):
    ...
    # Top phase bar
    bar_col = PHASE_BAR_COLORS.get(phase, (20,20,50))
    draw.rectangle([0, 0, W, 112], fill=bar_col)
...
# ─────────────────────────────────────────────────────────────────
# VOICE  (1.5x speed, deep male Madhur)
# ─────────────────────────────────────────────────────────────────
...
with st.sidebar:
    ...
    broad      = st.text_input("Medical Topic:", placeholder="e.g. Heart Failure")
...
topic = st.text_input(
    "💉 Medical Topic:",
    placeholder="e.g. Heart Failure ke 3 khamoosh signs jo sab ignore karte hain"
)
...
Write a SUSPENSE STORY video script that grips viewers from start to finish.

Follow this structure EXACTLY:
{phases_str}

Phase writing rules:
- HOOK: One shocking/scary opening fact. Viewer freezes instantly.
- MYSTERY: Something is wrong but we hide the answer. Build dread.
- CLUE: Drop one small clue. Viewer gets more curious.
- RISING: Tension peaks. Viewer now suspects the answer.
- REVEAL: THE BIG DRAMATIC TRUTH revealed at last.
- RESOLUTION: Doctor's advice, hope, and call to action for Dr. Vasu Memorial Clinic.

CRITICAL: Write EXACTLY {num_scenes} scene lines using this EXACT format:
SCENE [number] | [PHASE] | [3-5 word English medical keyword] | [1-2 Hindi sentences]

Example:
SCENE 1 | HOOK | Silent Heart Attack Warning | Kya aap jaante hain ki har 4 mein se 1 heart attack bilkul khamoshi se aata hai bina kisi dard ke?
SCENE 2 | MYSTERY | Heart Failure Signs | Lekin yeh khamoshi asliyat mein ek bada khatara chhupa rahi hai jo dheere dheere aapke dil ko andar se khatam kar rahi hai.

Write all {num_scenes} SCENE lines now (no other text between them):

ALSO ADD AFTER THE SCENES:
[HOOK_LINE] 5-word viral Hindi opening sentence
[CAPTION] 3-4 emotional Hindi lines for Instagram
[HASHTAGS] 20 hashtags (mix Hindi + English medical)
[THUMBNAIL] 4 bold Hindi words for thumbnail"""
            ...
            # ── BULLETPROOF PARSER ──
            scenes = parse_scenes(raw, phase_list)
            st.session_state.scenes = scenes
            n = len(scenes)
            status.update(label=f"✅ {n} scenes parsed successfully!", state="complete")
...
if st.session_state.package:
    ...
    if n > 0:
        st.markdown(f"""
        <div class="mrow">
          <div class="mbox"><div class="v">{n}</div><div class="l">Scenes Parsed</div></div>
          <div class="mbox"><div class="v">1.5x</div><div class="l">Voice Speed</div></div>
          <div class="mbox"><div class="v">16</div><div class="l">Cartoon Types</div></div>
          <div class="mbox"><div class="v">₹0</div><div class="l">Total Cost</div></div>
        </div>""", unsafe_allow_html=True)

        with st.expander("🎬 Story Arc Preview — All Scenes"):
        ...
        # ── Show debug info so user can report issue ─────────────────
        st.error("⚠️ 0 scenes parsed. Showing raw AI output for debug:")
        st.markdown('<div class="debug-box">' + st.session_state.raw_llm[:2000] + '</div>', unsafe_allow_html=True)
        st.info("💡 Try clicking Generate again — the AI sometimes changes its format.")


# ─────────────────────────────────────────────────────────────────
# STAGE 2 — PRODUCTION ENGINE
# (shows whenever scenes exist)
# ─────────────────────────────────────────────────────────────────
if st.session_state.scenes:
    st.markdown('<div class="card"><span class="badge">STAGE 2 — CARTOON PRODUCTION ENGINE</span>', unsafe_allow_html=True)
    st.info("🎨 16 unique cartoons · 🎙️ 1.5x Male Madhur voice · 🎥 Ken Burns · 📝 Hindi subtitles")
    ...
    if st.button("🎬 LAUNCH SUSPENSE VIDEO PRODUCTION"):
        ...
        try:
            for idx, sc in enumerate(scenes):
                ...
                logbox.write(f"   ✅ Cartoon ready.")

                # 2. Voice at 1.5x
                logbox.write(f"🎙️ Voice (1.5x Madhur): {sc['hindi'][:40]}...")
                aud_path = f"{PROJECT_DIR}/aud/s{idx}.mp3"
                asyncio.run(tts(sc["hindi"], aud_path))
                dur = get_dur(aud_path)
                logbox.write(f"   ✅ Audio: {dur:.1f}s")
                ...
                # 5. Preview (emoji computed outside f-string — no syntax error)
                if show_prev:
                    preview_title = emoji + " Scene " + str(idx+1) + " — " + sc["keyword"]
